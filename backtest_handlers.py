    ...
    try:
        ...
        for htf in HTF_LIST:
            ...
            for end_idx in range(LOOKBACK_CANDLES, len(htf_list_bt)):
                ...
                for zone in zones:
                    zone_key = (zone["type"], round(zone["top"],
8), round(zone["bottom"], 8))
                    ...
                    for c in future:
                        if zone["type"] == "bullish":
                            ...
                            if c["high"] >= invalidation:
                                outcome = "loss"; break
                    all_results.append({"htf": htf, "zone_type":
zone["type"], "outcome": outcome})
        if not all_results:
            return f"Backtest {symbol} ({months} bln): tidak ada
sinyal."
